...
- Claude Code 的 `<synthetic>` 行仅在四类 Token、Web Search 次数和费用全部为零时排除出模型展示与计费完整性判断；纯零用量任务的配置价格费用为 `$0.00`，不能阻断周期、项目、状态、会话、设备或任务费用。Synthetic 错误回复不能覆盖 `agent_connected.runtime.model`，旧库派生的 synthetic 主模型应在启动时从事件重建。原始 `turn_model_usage` 继续保留，任何非零 synthetic 用量必须显式展示，不能静默过滤。
...
- 当天不写入永久完成标记；启动和周期任务要继续刷新当天重叠窗口。首轮同步后，周期任务以最近一次成功窗口的结束时间为游标向前滑动并保留 overlap；进程重启时回退到最新逻辑事件时间加 overlap。overlap 必须覆盖可接受的最大 TLS 入库延迟，超过该窗口的极晚日志通过历史同步补回。旧 `fetched=0` 且未通过回退验证的日期必须重新检查。
...
- 趋势图使用双 Y 轴：任务数和工具调用共用左轴，Token 使用右轴，图例必须标明轴归属。模型、计费、工具、任务状态和数据质量面板允许鼠标、触摸和键盘排序，顺序仅保存在浏览器 `localStorage`。
- 模型 API P95 与部署成功率旁保留可聚焦的口径提示。前者是 SDK `duration_api_ms` 的第 95 百分位，不是 HTTP 延迟；后者是部署成功任务数除以出现部署开始或结束事件的任务数。
