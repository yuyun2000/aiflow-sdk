...
Keep model/core credentials out of anonymous browsers, make the public web entrypoint a server-side signed BFF, cap anonymous mechanical use by session/IP/global quotas, and expose meaningful safe Agent/SDK lifecycle events without disclosing hidden reasoning or secrets.
...
- [x] Redact hidden reasoning, credentials, paired identifiers, and absolute workspace paths
...
- [x] Filter redundant input/signature/thinking fragments, throttle Agent activity writes, and cap the raw-event DOM without dropping public text or complete tool calls/results
