...
- **刷新预算**：普通数据动画从 20 到 30 FPS 起步；连续整帧 Canvas、网络仪表或小屏从 10 到 20 FPS 起步；EPD 只做事件触发刷新；矩阵从 8 到 15 FPS 起步。
...
适合连续整帧且不能展示尚未画完 buffer 的动画。后缓冲先完成一帧，再切换隐藏标志或一次呈现；普通低频图形不要为此额外分配第二个 buffer。可参考 [hourglass LVGL sample](../../uiflow2-coder/assets/examples/m5ui/hourglass_320x240.py)。
...
- 每个动画记录触发者、目标对象、初始状态、持续时间、结束状态、重复次数和取消条件。
- 页面进入、按钮点击、传感器阈值等触发必须在代码中真实可达；不要留下从不执行的
  动画函数或永远为假的 request flag。
- 不让事件回调、主循环和传感器路径同时写同一位置/颜色/透明度。统一由状态机决定
  当前视觉值。
- pressed/selected 动效结束后必须回到稳定状态；除非任务本身需要，不使用无限循环
  的呼吸、闪烁或扫描效果。
- 看起来可操作的控件必须有经过文档确认的事件回调，或者以 disabled 样式明确不可用。
- 静态截图只能审查动画前后构图，不能证明节奏、无闪烁或帧率。没有模拟器预览或设备
  观察时，将动画视觉结果标记为 `PARTIAL`/`NOT RUN`。
